...
describe("Library", () => {
  ...
  beforeEach(() => {
    store = mockStore({ currencies: currencies, favoriteCurrencies: favoriteCurrencies });
  ...
  it("CurrenciesListItem - USD currency exists", () => {
    const item = wrapper.find(CurrenciesListItem);
    expect(item.at(0).find(".CurrenciesListItem__column").at(2).text()).toEqual("USD");
  });

  it("CurrenciesListItem - EUR currency exists", () => {
    const item = wrapper.find(CurrenciesListItem);
    expect(item.at(1).find(".CurrenciesListItem__column").at(2).text()).toEqual("EUR");
  });
